...
import SeventhRobotChatBox from "./SeventhRobotChatBox"

interface SeventhRobotProps {
...
const SeventhRobot = memo(function SeventhRobot({ onChatToggle, isOtherChatOpen, isFloating = false }: SeventhRobotProps) {
  // Mobile detection
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 1,
      text: "Merhaba, ben **SLM-X SidrexGPT**. Gelişmiş **dil modeli** teknolojimle size en **akıllı** ve **hızlı** yanıtları sunmak için buradayım!",
      isUser: false,
      timestamp: new Date(),
      status: 'ok'
    },
  ...
  const { sendMessage: sendChatMessage, loading: chatLoading } = useRobotChat('slmx')
  ...
  const toggleChat = useCallback(() => {
    ...
    handleChatToggle("seventh", newChatState)
    ...
    <div className="relative">
      <button
        ...
        className={`w-24 h-24 rounded-full shadow-2xl hover:shadow-xl transition-all duration-300 transform hover:scale-105 cursor-pointer flex items-center justify-center relative overflow-visible ${
          isChatOpen || isHovered ? "seventh-robot-active" : ""
        }`}
        style={{
          backgroundColor: "#FFFFA1",
          border: "2px solid #8EE21B",
        }}
      >
        {/* Seventh Robot Mascot */}
        <div className={`robot-mascot-container ${isChatOpen || isHovered ? "seventh-active" : ""}`}>
          <div className="robot-antenna-seventh"></div>
          <div className="robot-head-seventh">
            <div className="robot-ear-seventh robot-ear-left"></div>
            <div className="robot-ear-seventh robot-ear-right"></div>
            <div className="robot-face-seventh">
              <div className="robot-seventh-eyes"></div>
              <div className="robot-seventh-hover-eyes"></div>
              <div className="robot-seventh-mouth"></div>
            </div>
          </div>
        </div>
        <div className="robot-seventh-box">
          <div className="robot-seventh-logo"></div>
        </div>
        <div className="robot-seventh-pineapple-static"></div>
        <div className="robot-seventh-pineapple-left"></div>
      </button>
      ...
      {isChatOpen && (
        <SeventhRobotChatBox
          ...
          onClose={() => {
            ...
            notifyCloseChatbox()
            handleChatToggle("seventh", false)
          }}
