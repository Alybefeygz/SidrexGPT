...
interface SeventhRobotChatBoxProps {
  messages: Message[]
  inputValue: string
...
export default function SeventhRobotChatBox({
  messages,
  inputValue,
  setInputValue,
  sendMessage,
  ...
  return (
    ...
    >
      {/* Chat Header - with green theme */}
      <div
        className="text-white p-5 rounded-t-lg flex justify-between items-center"
        style={{ backgroundColor: "#8EE21B" }}
      >
        <h3 className="font-semibold text-lg">SLM-X SidrexGPT</h3>
          ...
          {messages.map((message) => (
            <div key={message.id} className={`flex ${message.isUser ? "justify-end" : "justify-start"}`}>
              <div
                ...
                style={message.isUser ? { backgroundColor: "#8EE21B" } : {}}
              >
      ...
      <div className="p-5 border-t border-gray-200 bg-white rounded-b-lg">
        <div className="flex items-center space-x-3">
          ...
          <button
            ...
            style={{ backgroundColor: "#FFEF00", color: "#8EE21B" }}
          >
            <Send size={18} />
          </button>
        </div>
