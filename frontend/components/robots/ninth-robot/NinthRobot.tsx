...
import NinthRobotChatBox from "./NinthRobotChatBox"

interface NinthRobotProps {
...
const NinthRobot = memo(function NinthRobot({ onChatToggle, isOtherChatOpen, isFloating = false }: NinthRobotProps) {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 1,
      text: "Merhaba, ben **Alyuvar SidrexGPT**. **Alyuvar** ürününün özel asistanıyım!",
      isUser: false,
      timestamp: new Date(),
      status: 'ok'
    },
  ...
  const { sendMessage: sendChatMessage, loading: chatLoading } = useRobotChat('alyuvar')
  ...
  const toggleChat = useCallback(() => {
    ...
    handleChatToggle("ninth", newChatState)
    ...
    <div className="relative">
      <button
        ...
        className={`w-24 h-24 rounded-full shadow-2xl hover:shadow-xl transition-all duration-300 transform hover:scale-105 cursor-pointer flex items-center justify-center relative overflow-visible ${
          isChatOpen || isHovered ? "ninth-robot-active" : ""
        }`}
        style={{
          backgroundColor: "#FFE0CC",
          border: "3px solid #E07E73",
        }}
      >
        {/* 8. Robot - Bağımsız CSS Sınıfları */}
        <div className={`robot-mascot-container ${isChatOpen || isHovered ? "ninth-active" : ""}`}>
          <div className="robot-antenna-ninth"></div>
          <div className="robot-antenna-ninth-hover"></div>
          <div className="robot-head-ninth">
            <div className="robot-ear-ninth robot-ear-left"></div>
            <div className="robot-ear-ninth robot-ear-right"></div>
            <div className="robot-face-ninth">
              <div className="robot-ninth-eyes"></div>
              <div className="robot-ninth-hover-eyes"></div>
              <div className="robot-ninth-mouth"></div>
              <div className="robot-ninth-hover-mouth"></div>
            </div>
          </div>
        </div>
        
        {/* Alyuvar mascots - independent from robot head movements */}
        <div className="robot-ninth-maskot"></div>
        <div className="robot-ninth-hover-maskot"></div>
      </button>
