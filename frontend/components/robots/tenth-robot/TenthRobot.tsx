...
import TenthRobotChatBox from "./TenthRobotChatBox"

interface TenthRobotProps {
...
const TenthRobot = memo(function TenthRobot({ onChatToggle, isOtherChatOpen, isFloating = false }: TenthRobotProps) {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 1,
      text: "Merhaba, ben **Kabak Çekirdeği SidrexGPT**. **Kabak Çekirdeği** ürününün özel asistanıyım!",
      isUser: false,
      timestamp: new Date(),
      status: 'ok'
    },
  ...
  const { sendMessage: sendChatMessage, loading: chatLoading } = useRobotChat('kabak-cekirdegi')
  ...
  const toggleChat = useCallback(() => {
    ...
    handleChatToggle("tenth", newChatState)
    ...
    <div className="relative">
      <button
        ...
        className={`w-24 h-24 rounded-full shadow-2xl hover:shadow-xl transition-all duration-300 transform hover:scale-105 cursor-pointer flex items-center justify-center relative overflow-visible ${
          isChatOpen || isHovered ? "tenth-robot-active" : ""
        }`}
        style={{
      ...
      >
        ...
        <div className={`robot-mascot-container ${isChatOpen || isHovered ? "tenth-active" : ""}`}>
          <div className="robot-antenna-tenth"></div>
          <div className="robot-head-tenth">
            <div className="robot-ear-tenth robot-ear-left"></div>
            <div className="robot-ear-tenth robot-ear-right"></div>
            <div className="robot-face-tenth">
              <div className="robot-tenth-eyes"></div>
              <div className="robot-tenth-hover-eyes"></div>
              <div className="robot-tenth-mustache"></div>
            </div>
          </div>
        </div>
        
        {/* Kabak Çekirdeği mascots - independent from robot head movements */}
        <div className="robot-tenth-kabak-cekirdegi"></div>
        <div className="robot-tenth-hover-kabak-cekirdegi"></div>
      </button>
