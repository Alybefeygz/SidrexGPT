...
export default function FifthRobotPage() {
  const [activeChatRobot, setActiveChatRobot] = useState<boolean>(false)
  const { canEditPDF, getUserPermissions } = useAuth()
  
  // Robot'u slug ile al
  const { data: robotData, loading: robotLoading, error: robotError } = useRobotBySlug('milk-thistle')
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <div className="text-sm text-gray-500 mt-2">Geçersiz robot slug'ı: milk-thistle</div>
          <div className="text-xs text-gray-400 mt-4">Debug Data: {JSON.stringify(robotData)}</div>
        </div>
      </div>
    )
      ...
      <div className="min-h-screen bg-white">
        ...
        <div className="bg-gray-50 py-3">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <nav className="text-sm">
              ...
              <span className="mx-2 text-gray-400">{">"}</span>
              <span className="text-gray-900">Milk Thistle Complex</span>
            </nav>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex flex-col items-center justify-center">
            <h1 className="text-3xl font-bold mb-8" style={{ color: "#A863C1" }}>Milk Thistle Complex Asistanı</h1>
